# This file compares joint LMM to simple linear regression and univariate LMM.
...
#source("C:\\Users\\hallb\\R_projects\\mixed models examples\\simulate_data_Analys2.R") # use this if you want to simulate another data set
load("simulated_data.RData")

df <- simulated_data # simulate_data()
...
fixedEffect_int_slr <- 0
fixedEffect_slope_slr <- 0

for (i in 1:N) {
  slr_model <- lm(FEV1 ~ Age, data = df %>% filter(ID == i, Setting == 2))
  fixedEffect_int_slr <- fixedEffect_int_slr + slr_model$coefficients[1]
  fixedEffect_slope_slr <- fixedEffect_slope_slr + slr_model$coefficients[2]
}
fixedEffect_int_slr <- fixedEffect_int_slr / N
fixedEffect_slope_slr <- fixedEffect_slope_slr / N
...
for (i in 1:N) {
  ...
  # fixed effects, modify data means that we delete/slice last hospital measurement of individual i and efterwards we extract all hospital data.
  slr_hospital_modifiedData <- lm(FEV1 ~ Age, data = (df %>% slice(-indexToSlice)) %>% filter(Setting == 2))
  mspe4_slr <- mspe4_slr + (fixedEffect_int_slr-(slr_hospital_modifiedData$coefficients[1]))^2
  mspe4_slr_slope <- mspe4_slr_slope + (fixedEffect_slope_slr-(slr_hospital_modifiedData$coefficients[2]))^2
}
...
resultsAnalys2 <- tibble(
  name = c("msePRED1", "msePRED2", "msePRED3 int", "msePRED3 slope", "mseEST4 int", "mseEST4 slope"),
...
save(resultsAnalys2, file = "resultsAnalys2.RData")



# Why is slr error so big? Im not sure there is an error. 
# Below I check some numbers
# slice deletes the row. And I know the indices right now. slice(-10) means that the 10 row is deleted
# Because of df_copy I know the indices.
lm(FEV1 ~ Age, data = (df %>% slice(-10)) %>% filter(ID == 1, Setting == 2))$coefficients[1]
lm(FEV1 ~ Age, data = (df %>% filter(ID == 1, Setting == 2)))$coefficients[1]

lm(FEV1 ~ Age, data = (df %>% slice(-23)) %>% filter(ID == 2, Setting == 2))$coefficients[1]
lm(FEV1 ~ Age, data = (df %>% filter(ID == 2, Setting == 2)))$coefficients[1]

lm(FEV1 ~ Age, data = (df %>% slice(-36)) %>% filter(ID == 3, Setting == 2))$coefficients[1]
lm(FEV1 ~ Age, data = (df %>% filter(ID == 3, Setting == 2)))$coefficients[1]

...
simulate_data <- function(
  ...
  return (list(df = df, df_modified = df_modified, beta_true = beta, b_true = b))
}

# Here you can change all the variables as you wish
xmin <- 7
xmax <- 8
sigma_beta0 <- 1
sigma_beta1 <- 1
...
size <- 1 # if 1, then I delete last point on all individuals. 
...
data <- simulate_data(
    ...
    size)
df <- data$df
df_modified <- data$df_modified


# Current value, subject_specific_mean, subject_specific_trend
# Delete the last point for all subjects. Fit the model. Make it simple for now. 

joint_lmm <- lmer(FEV1 ~ Setting + Age + Setting * Age + (-1 + as.factor(Setting) + Age:as.factor(Setting) | ID), data = df) 
joint_lmm_modified <- lmer(FEV1 ~ Setting + Age + Setting * Age + (-1 + as.factor(Setting) + Age:as.factor(Setting) | ID), data = df_modified) 

current_value <- tibble()
for (i in 1:nrow(df)) {
  row <- (df %>% filter(ID == i, Setting == 2)) %>% slice(4)
  current_value <- bind_rows(current_value, row)
}
new_data <- data.frame(ID = (df %>% distinct(ID)), Setting = rep(2, N*size), Measurement_nr = rep(4, N*size), Age = current_value$Age)
y_hat_ij <- predict(joint_lmm_modified, newdata = new_data, re.form = NA) # actually y_hat_i,-j, Confusing? I know. 
plot(current_value$Age, (current_value$FEV1 - y_hat_ij))
mse <- mean((current_value$FEV1 - y_hat_ij)^2)
print(mse)

# Subject-specific mean at a specific point in time
y <- predict(joint_lmm, newdata = new_data, re.form = NA)
plot(current_value$Age, (y - y_hat_ij))
mse <- mean((y-y_hat_ij)^2)
print(mse)

# Subject specific trend
plot(BLUPjointlmm$ID - BLUPjointlmmmodified$ID)

# Population average trend
print(fixef(joint_lmm)-fixef(joint_lmm_modified))


### Checking BLUP for different N.
Ns <- c(50, 100, 150, 200)
mses <- c()
...
for (i in 1:length(Ns)) {
  ...
  data <- simulate_data(
    ...
    cov_homeb1hospb1,
    size)
  joint_lmm <- lmer(FEV1 ~ Setting + Age + Setting * Age + (-1 + as.factor(Setting) + Age:as.factor(Setting) | ID), data = data$df)
  mses <- c(mses, mean((predict(joint_lmm)-data$df$FEV1)^2) )
}
...
ggplot(my_tibble, aes(x = N, y = MSE)) + 
  geom_point() + 
